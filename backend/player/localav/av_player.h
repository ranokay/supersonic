...
// Default shared-mode output. Exclusive/hog mode reconfigures the device to
// match the decoded stream as closely as the host API allows.
#define AVPLAYER_DEFAULT_SAMPLE_RATE        48000
#define AVPLAYER_DEFAULT_SAMPLE_RATE_STR    "48000"
#define AVPLAYER_DEFAULT_CHANNELS           2
// Ring buffer capacity in seconds.
#define AVPLAYER_RING_SECONDS   4
...
typedef struct {
    ...
    int  exclusive_ready;
    int  lossy;
    int  physical_format_count;
    int  exclusive_format_count;
    int  min_sample_rate;
    int  max_sample_rate;
    int  max_bit_depth;
    int  channels;
    char active_format[128];
    char capability_summary[256];
} av_device_info_t;
